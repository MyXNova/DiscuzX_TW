...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? '全螢幕' : '取消全螢幕';
				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? '恢復編輯器大小' : '全螢幕方式編輯');};
				buttons[i].onclick = function(e) {editorfull();doane();}
			} else if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'simple') {
				buttons[i].innerHTML = !simplodemode ? '常用' : '進階';
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('您的瀏覽器不支援此功能，請升級瀏覽器版本', 'notice', '友情提示');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="點選開啟自動儲存" href="javascript:;" onclick="setAutosave()">開啟自動儲存</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip('資料已於 ' + h + '：' + m + ' 儲存');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="點選關閉自動儲存" href="javascript:;" onclick="setAutosave()">' + savedatac + ' 秒後儲存</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? '資料自動儲存已開啟' : '資料自動儲存已關閉');
...
function editorfull(op) {
	...
	$(editorid + '_fullswitcher').innerHTML = editorisfull ? '取消全螢幕' : '全螢幕';
	initesbar();
}
...
function checklength(theform) {
	...
	showDialog('目前長度： ' + mb_strlen(message) + ' 字元，' + (postmaxchars != 0 ? '系統限制： ' + postminchars + ' 到 ' + postmaxchars + ' 字元。' : ''), 'notice', '字數檢查');
}
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'inserthorizontalrule', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'beginning', 'attach', 'image', 'pasteword', 'index', 'postbg', 'password']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		showDialog('<div id="remotedowninfo"><p class="mbn">正在下載遠端附件，請稍等……</p><p><img src="' + STATICURL + 'image/common/uploading.gif" alt="" /></p></div>', 'notice', '', null, 1);
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = '請輸入連結網址：<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />請輸入連結文字：<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'postbg':
				...
				break;
			case 'password':
				str = '<p class="pbn">請輸入文章密碼： <input type="text" id="' + ctrlid + '_param_1" size="10" value="" class="px" /></p>';
				break;
			case 'code':
				if(wysiwyg) {
			...
			case 'tbl':
				...
				break;
			case 'aud':
				str = '<p class="pbn">請輸入音樂檔案網址：</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">支援 wma mp3 ra rm 等音樂格式<br />示例：http://server/audio.wma</p>';
				break;
			case 'vid':
				str = '<p class="pbn">請輸入視訊影片網址：</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">寬：<input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; 高：<input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">支援 Youtube、優酷、土豆、56、酷6 等視訊網站的視訊影片網址<br />支援 wmv avi rmvb mov swf flv 等視訊影片格式<br />示例：http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">請輸入 Flash 檔案網址：</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">寬：<input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; 高：<input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">支援 swf flv 等 Flash 網址<br />示例：http://server/flash.swf</p>';
				break;
			case 'beginning':
				str = '<p class="pbn">請輸入開頭動畫 Flash 或 圖片 網址：</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">點選連結網址：</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
				...
				str += '<p class="pbn">讀取、消失的效果： </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />無 &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />淡入淡出 &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />展開閉合</p>';
				str += '<p class="xg2 pbn">支援 swf flv jpg gif png 網址<br />寬高範圍：寬 400~1024 高 300~640<br />示例：http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = '從 Word 貼上內容';
				str = '<p class="px" style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">請透過快速鍵(Ctrl+V)把 Word 檔案中的內容貼到上方</p>';
				menuwidth = 600;
				menupos = '00';
				menutype = 'win';
				break;
			case 'index':
				...
				str = '<p class="pbn">[index]<br />\n\
					[#<span class="xi1">頁碼</span>]<span class="xi1">標題</span> &nbsp;&nbsp;<span class="xg1">跳轉到指定的頁數</span><br />\n\
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>]<span class="xi1">標題</span> &nbsp;&nbsp;<span class="xg1">跳轉到指定的文章</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">頁碼</span> &nbsp;&nbsp;<span class="xg1">用 [page] 對目前文章分頁後的頁碼</span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">文章的 TID 和 PID</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">增加行首縮排</span></p>';
				break;
			default:
		...
		if(menupos == '00') {
			...
			s = '<table width="100%" cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c">'
				...
				+ '<p class="o pns"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>送出</strong></button></p>'
				+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		} else {
			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">關閉</a></span><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>送出</strong></button></div></div>';
		}
		menu.innerHTML = s;
