...
class ucclient_db {
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname = '', $dbcharset = '', $pconnect = 0, $tablepre='', $time = 0) {
		if (intval($pconnect) === 1) $dbhost = 'p:' . $dbhost; // 前面加「p:」，表示 persistent connection
		$this->dbhost = $dbhost;
		$this->dbuser = $dbuser;
		$this->dbpw = $dbpw;
		$this->dbname = $dbname;
	...
	function query_stmt($sql, $key = array(), $value = array(), $type = '', $saveprep = FALSE, $cachetime = FALSE) {
		...
		// SELECT 指令返回陣列供其他方法使用, 其他情況返回 SQL 執行結果
	...
	function parse_query($sql, $key = array(), $value = array()) {
		$list = '';
		$array = array();
		if(strpos($sql, '?')) {// 如果 SQL 存在問號則使用傳統匹配方式，KEY 順序與「?」的順序維持一致
			foreach ($key as $k => $v) {
		...
		} else {// 不存在問號則使用類比 PDO 模式，允許在 SQL 內指定變數名
