...
if($op=='magic') {
	...
	$db_target->query("UPDATE ".$db_target->table('common_magic')." SET name='變色卡', description='可以將文章或網誌的標題醒目提示，變更顏色' WHERE identifier='highlight'");
	$db_target->query("UPDATE ".$db_target->table('common_magic')." SET name='顯身卡', description='可以檢視一次匿名會員的真實身份。' WHERE identifier='namepost'");
	...
	showmessage("繼續轉換資料表 ".$table_source." 會員道具", "index.php?a=$action&source=$source&prg=$curprg&op=usermagic&start=0");
	...
	if($done == false) {
		showmessage("繼續轉換資料表 ".$table_source." 會員道具-> $nextid", "index.php?a=$action&source=$source&prg=$curprg&op=usermagic&start=$nextid");
	} else {
		showmessage("繼續轉換資料表 ".$table_source." 道具收入記錄->0", "index.php?a=$action&source=$source&prg=$curprg&op=magicinlog&start=0");
	}
	...
	if($done == false) {
		showmessage("繼續轉換資料表 ".$table_source." 道具收入記錄-> $nextid", "index.php?a=$action&source=$source&prg=$curprg&op=magicinlog&start=$nextid");
	} else {
		showmessage("繼續轉換資料表 ".$table_source." 道具使用記錄->0", "index.php?a=$action&source=$source&prg=$curprg&op=magicuselog&start=0");
	}
	...
	if($done == false) {
		showmessage("繼續轉換資料表 ".$table_source." 道具使用記錄-> $nextid", "index.php?a=$action&source=$source&prg=$curprg&op=magicuselog&start=$nextid");
	}
}
