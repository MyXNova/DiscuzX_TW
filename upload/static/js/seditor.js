	...
	if(!$(menuid)) {
		switch(tag) {
			case 'at':
				...
				str = '請輸入會員名稱：<br /><input type="text" id="atkeyword" style="width:240px" value="" class="px" onkeydown="atFilter(this.value, \'at_list\',\'atListSet\',event);" /><div class="p_pop" id="at_list" style="width:250px;"><ul><li>@朋友帳號，就能提醒他來閱讀文章</li></ul></div>';
			...
			case 'code':
			case 'quote':
				var tagl = {'quote' : '請輸入要插入的引用', 'code' : '請輸入要插入的程式碼'};
			...
			case 'img':
				str = '請輸入圖片網址：<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" onchange="loadimgsize(this.value, \'' + seditorkey + '\',\'' + tag + '\')" />' +
					'<p class="mtm">寬(選填)：<input type="text" id="' + ctrlid + '_param_2" style="width: 15%" value="" class="px" /> &nbsp;' +
					'高(選填)：<input type="text" id="' + ctrlid + '_param_3" style="width: 15%" value="" class="px" /></p>';
