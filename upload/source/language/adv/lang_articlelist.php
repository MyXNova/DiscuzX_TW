<?php

/**
 *      [Discuz!] (C)2001-2099 Comsenz Inc.
 *      This is NOT a freeware, use is subject to license terms
 *
 *      $Id: lang_articlelist.php 27449 2012-02-01 05:32:35Z zhangguosheng $
 */

if(!defined('IN_DISCUZ')) {
	exit('Access Denied');
}

$lang = array
(
	'articlelist_name' => '門戶 文章列表廣告',
	'articlelist_desc' => '展現方式：文章廣告顯示於頻道文章分類的文章列表頁面，分為頂端、底端 2 個投放位置。',
	'articlelist_position' => '投放位置',
	'articlelist_position_comment' => '分為 4 個位置，上下各有 2 個緊鄰的顯示位置',
	'articlelist_position_up1' => '頂端(上)',
	'articlelist_position_up2' => '頂端(下)',
	'articlelist_position_down1' => '底端(上)',
	'articlelist_position_down2' => '底端(下)',
	'articlelist_category' => '投放頻道分類',
	'articlelist_category_comment' => '設定廣告投放的頻道分類',
);

?>