...
class base {
	...
	function detectescape($basepath, $relativepath) {
		...
		// 如果檔案或資料夾不存在，有可能是新增前的檢查，使用其上一級路徑
		if(!file_exists($basepath . $relativepath)) {
			$relativepath = dirname($relativepath);
			// 上一級還不存在，按最壞情況處理，阻止請求
			// 不區分返回值的目的也是為了避免給攻擊者有價值的資訊
			if(!file_exists($basepath . $relativepath)) {
				return FALSE;
			}
		}
		...
		// $real_base 與 $real_target 相等，表示就是在瀏覽 base 目錄，允許
		// 或者
		// $real_target 的開頭就是 $real_base，表示在瀏覽 base 之下的檔案 / 資料夾，允許
