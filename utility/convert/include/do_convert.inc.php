...
if(empty($process)) {
	showmessage("請首先選擇轉換程式", "index.php?action=select&source=$source");
}
...
foreach (array('start', 'tables', 'steps') as $program) {
	if(!empty($process[$program]) && !$process[$program.'_is_end']) {
		...
		if($prg_next) {
			...
			$timetodo = "升級開始時間：<strong>$stime</strong>, 升級程式已經執行了 <strong>$rday</strong> 天 <strong>$rhour</strong> 小時 <strong>$rmin</strong> 分 <strong>$rsec</strong> 秒";
			$timetodo .= "<br><br>目前正在執行轉換程式( $prg_done / $prg_total ) <strong>$prg</strong>，轉換過程中需要多次跳轉，請勿關閉瀏覽器。";
			$timetodo .= "<br><br>如果程式中斷或者需要重新開始目前程式，請點選 (<a href=\"index.php?a=convert&source=$source&prg=$prg\">重新開始</a>)";
			...
			if(file_exists($prg_dir[$program].$prg)) {
				...
				showmessage("轉換程式 $prg 執行完畢， 現在跳轉到下一個程式", "index.php?a=convert&source=$source", null, 500);
			} else {
				showmessage('資料轉換中斷！ 無法找到轉換程式 '.$prg);
			}
		} else {
...
showmessage('轉換程式全部執行完畢', "index.php?action=finish&source=$source");
