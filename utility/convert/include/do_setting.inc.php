...
if(!submitcheck('submit')) {
	show_form_header();
	show_table_header();
	show_table_row(array(array('colspan="3"', '編輯配置檔 source/'.$source.'/setting.ini')), 'title');
	...
	foreach ($setting as $key => $value) {
		if(is_array($value) && !empty($value)) {
			...
			foreach ($value as $k => $v) {
				...
				lang($k),
				array('class="bg2" width="70%"', '<input type="text" size="40" name="newsetting['.$key.']['.$k.']" value="'.htmlspecialchars($v).'"'.$disabled.'>'.($disabled ? ' 唯讀' : ''))
				), 'bg2'
				);
			}
		}
	...
	if(is_array($newsetting)) {
		...
		if($fp = @fopen($sfile, 'w')) {
			fwrite($fp, implode("\n", $lines));
			fclose($fp);
			showmessage('設定已經更新完畢並成功儲存', 'index.php', '', 1000);
		} else {
			showmessage('該設定檔案為唯讀檔案，無法儲存，請返回');
		}
	}
	showmessage('您沒有改變任何設定', 'index.php', '', 1000);
}