...
function extrafunc_atListMenu(tag, op) {
	...
	if(!op) {
		...
		return '請輸入會員名稱：<br /><input type="text" id="atkeyword" style="width:240px" value="" class="px" onkeydown="atEnter(event, \'atListSet\')" onkeyup="atFilter(this.value, \'at_list\',\'atListSet\',event, true);" /><div class="p_pop" id="at_list" style="width:250px;"><ul><li>@朋友帳號，就能提醒他來閱讀文章</li></ul></div>';
	} else {
		if($('atkeyword').value) {
...
function atMenu(x, y) {
	...
	$('at_menu').innerHTML = '<div class="loadicon vm"></div> 請稍候... ';
}
...
function atFilter(kw, id, call, e, nae) {
	...
	if(nae || !atEnter(e, call)) {
		...
		if(atResult.length) {
			...
			$(id).innerHTML = '<ul>' + newlist + '<li class="xg1">@朋友帳號，就能提醒他來閱讀文章</li></ul>';
		} else {
