...
if(submitcheck('submit')) {
	...
	if(is_array($prgs)) {
		...
		showmessage("您選擇了 （".count($prgs)."） 個轉換程式，下面開始轉換", "index.php?a=convert&s=$source");
	}
}

showtips("<li>一般情況下，您需要執行下面所有資料表的轉換，除非您在執行過程中出現了意外的中斷或者有特殊需求，才進行必要的選擇</li>");
show_form_header();
show_table_header();
...
if($setting['start']) {
	show_table_row(array('轉換之前執行的程式'), 'bg1');
...
if($tablelist) {
	sort($tablelist);
	show_table_row(array('<span style="float: left">資料表轉換程式</span>'), 'bg1');
	echo "<tr class=bg2><td colspan=2>\n<ul id=\"ulist\">";
...
if($setting['steps']) {
	show_table_row(array('其他輔助轉換程式'), 'bg1');
	echo "<tr class=bg2><td colspan=2>\n<ul id=\"ulist\">";
