...
{
	/*
	 * 在 memory 啟用的時候，存儲於 common_banned_index 的 SortedSet 中
	 * member = ip 的 16 進位表達，score = 1 表示封禁，score = 0 表示不封禁
	 */
	...
	public function check_banned($time_to_check, $ip) {
		...
		if ($banned === false || !$this->_allowmem) { // 如果 memory 中沒有值，或不使用 memory，都走資料庫
			$iphex_val = '0x' . $iphex;
			$ret = DB::result_first(
