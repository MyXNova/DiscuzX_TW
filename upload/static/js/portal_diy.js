...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
	'blockDefaultClass' : [{'key':'選擇樣式','value':''},{'key':'無邊框且無邊距','value':'cl_block_bm'},{'key':'樣式 1','value':'xbs_1'},{'key':'樣式 2','value':'xbs xbs_2'},{'key':'樣式 3','value':'xbs xbs_3'},{'key':'樣式 4','value':'xbs xbs_4'},{'key':'樣式 5','value':'xbs xbs_5'},{'key':'樣式 6','value':'xbs xbs_6'},{'key':'樣式7','value':'xbs xbs_7'}],
	'frameDefaultClass' : [{'key':'選擇樣式','value':''},{'key':'無邊框且無邊距','value':'cl_frame_bm'},{'key':'無邊框框架','value':'xfs xfs_nbd'},{'key':'樣式 1','value':'xfs xfs_1'},{'key':'樣式 2','value':'xfs xfs_2'},{'key':'樣式 3','value':'xfs xfs_3'},{'key':'樣式 4','value':'xfs xfs_4'},{'key':'樣式 5','value':'xfs xfs_5'}],
	setDefalutMenu : function () {
		...
		this.addMenu('block', '資料', 'drag.openBlockEdit(event,"data")');
		this.addMenu('block', '更新', 'drag.blockForceUpdate(event)');
		this.addMenu('frame', '匯出', 'drag.frameExport(event)');
		this.addMenu('tab', '匯出', 'drag.frameExport(event)');
	},
	...
	openStyleEdit : function (e) {
		...
		var repeatarr = [{'key':'重複','value':'repeat'},{'key':'不重複','value':'no-repeat'},{'key':'橫向重複','value':'repeat-x'},{'key':'縱向重複','value':'repeat-y'}];
		var stylearr = [{'key':'無樣式','value':'none'},{'key':'實線','value':'solid'},{'key':'點線','value':'dotted'},{'key':'虛線','value':'dashed'}];
		var table = '<table class="tfm">';
		table += '<tr><th>字型</th><td><input type="text" id="fontsize" class="px p_fre vm" value="'+fontsize+'" size="2" />px <input type="text" id="fontcolor" class="px p_fre vm" value="'+fontcolor+'" size="2" />';
		table += getColorPalette(id+'_fontPalette', 'fontcolor' ,fontcolor)+'</td></tr>';
		table += '<tr><th>連結</th><td><input type="text" id="linkfontsize" class="px p_fre vm" value="'+linkfontsize+'" size="2" />px <input type="text" id="linkcolor" class="px p_fre vm" value="'+linkcolor+'" size="2" />';
		...
		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'">個別設定</label></p></td></tr>';
		...
		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">個別設定</label></p></td></tr>';
		...
		if (objType == 1) {
			...
			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> 個別設定</label></p></td></tr>';
		}
		...
		table += '<tr><th>指定 class</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';

		var wname = objType ? '模組' : '框架';
	...
	openTitleEdit : function (e) {
		...
		var titlename = obj instanceof Block ? '模組' : '框架';
		var repeatarr = [{'key':'重複','value':'repeat'},{'key':'不重複','value':'no-repeat'},{'key':'橫向重複','value':'repeat-x'},{'key':'縱向重複','value':'repeat-y'}];
		...
		if (obj instanceof Tab) {
			var switchArr = [{'key':'點選','value':'click'},{'key':'滑過','value':'mouseover'}];
		...
		li += '<tr><th>連結：</th><td><input type="text" id="titleLink_0" class="px p_fre" value="`link`" /></td></tr>';
		li += '<tr><th>圖片：</th><td><input type="text" id="titleSrc_0" class="px p_fre" value="`src`" /></td></tr>';
		li += '<tr><th>位置：</th><td><select id="titleFloat_0" class="ps vm"><option value="" `left`>靠左</option><option value="right" `right`>靠右</option></select>';
		li += '&nbsp;&nbsp;偏移量：<input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="2" />px</td></tr>';
		li += '<tr><th>字型：</th><td><select class="ps vm" id="titleSize_0" ><option value="">大小</option>`size`</select>';
		...
		var c = len + 1;
			html = '<div class="c diywin" style="width:450px;height:400px; overflow:auto;"><table cellspacing="0" cellpadding="0" class="tfm pns"><tr><th></th><td><button type="button" id="addTitleInput" class="pn" onclick="drag.addTitleInput('+c+');"><em>增加新標題</em></button></td></tr></table><div id="titleEdit">'+html+common+'</div></div>';
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('您確定要刪除嗎？刪除以後將不可恢復！')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>正在讀取內容...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('抱歉，您沒有權限新增或編輯模組！', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('您已經做過修改，請儲存後再做匯出，否則匯出的資料將不包括您這次所做的修改。');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = '共 <span style="color:blue">'+this.blocksLen+'</span> 個模組，正在更新第 <span style="color:red">'+cur+'</span> 個，已完成 <span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
	...
	blockForceUpdateBatch : function (blocks) {
		...
		showDialog('<div id="allupdate" style="width:350px;line-height:28px;">開始更新...</div>','confirm','更新模組資料', '', true, 'drag.endBlockForceUpdateBatch()');
		var wait = function() {
	...
	clearAll : function () {
		if (confirm('您確實要清空網頁上所在 DIY 資料嗎？清空以後將不可恢復！')) {
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("提示：未找到框架，請先新增框架。");spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
	...
	getTabHtml : function () {
		...
		var str = '<div id="'+id+'" class="'+className+'">';
		str += '<div id="'+id+'_title" class="'+titleClassName+'"><span class="'+this.titleTextClass+'">tab 標籤</span></div>';
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
			showDialog('<div style="line-height:28px;">按繼續按鈕將開啟暫存資料並 DIY，<br />按刪除按鈕將刪除暫存資料！</div>','confirm','是否繼續暫存資料的 DIY？', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', '繼續', '刪除');
...
spaceDiy.extend({
		...
		if (!optype) {
			if (['portal/portal_topic_content', 'portal/list', 'portal/view'].indexOf(tplpre[0]) == -1) {
				if (document.diyform.template.value.indexOf(':') > -1 && !document.selectsave) {
					...
					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />套用於此類全部網頁</label>\n\
					<label><input type="radio" value="1" name="savemod"'+dchecked+' />只套用於本網頁</label></form>','notice', '', spaceDiy.save);
					return false;
				}
	...
	cancel : function () {
		...
		if (drag.isClearClose) {
			showDialog('<div style="line-height:28px;">是否保留暫存資料？<br />按確定按鈕將保留暫存資料，按取消按鈕將刪除暫存資料！</div>','confirm','保留暫存資料', function(){location.href = spaceDiy.cancelDiyUrl();}, true, function(){window.onunload=function(){spaceDiy.cancelDIY()};location.href = spaceDiy.cancelDiyUrl();});
		} else {
