...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? '您確定要刪除該資料嗎？' : '您確定要過濾該資料嗎？';
	if(confirm(msg)) {
...
function toggleSettingShow() {
	if(!$('tbody_setting').style.display) {
		...
		$('a_setting_show').innerHTML = '展開設定項';
	} else {
		$('tbody_setting').style.display = '';
		$('a_setting_show').innerHTML = '收起設定項';
	}
	doane();
}
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('模組標識不能為空', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('自訂內容錯誤，HTML 語法：'+tag+' 標籤不相配', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('HTML 語法：'+tag+' 標籤不相配', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function strLenCalc(obj, checklen, maxlen) {
	...
	if(curlen <= maxlen) {
		checklen.innerHTML = '已輸入 <b>'+(curlen)+'</b> 個字元';
		return true;
	} else {
		checklen.innerHTML = '超出 <b style="color:red">'+(curlen - maxlen)+'</b> 個字元';
		return false;
	}
}
