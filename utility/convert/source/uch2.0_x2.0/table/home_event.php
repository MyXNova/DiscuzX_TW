...
if(empty($eventfid)) {
	$board_name = 'UCHome 資料';
	$forum_name = 'UCHome 活動';
...
while ($event = $db_source->fetch_array($event_query)) {
	...
	$activityarr = array(
		...
		'applynumber' => $event['membernum'] - 1,// Home 裡的活動成員包括建立者
...
if($nextid) {
	showmessage("繼續轉換資料表 ".$table_source." eventid> $nextid", "index.php?a=$action&source=$source&prg=$curprg&start=$nextid");
}
