...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
			errormessage(mailObj.id, '請輸入電子信箱');
		}
...
var emailMenuST = null, emailMenui = 0, emaildomains = ['gmail.com', 'hotmail.com', 'outlook.com', 'live.com', 'yahoo.com', 'yahoo.com.tw'];
...
function checkusername(id) {
	...
	if(username.match(/<|>|"|\(|\)|'/ig)) {
		errormessage(id, '會員名稱包含敏感字元！');
		return;
	}
	var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? '會員名稱不得小於 3 個字元！' : '會員名稱不得超過 15 個字元！');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	if(pwlength > 0) {
		if($(id1).value.length < pwlength) {
			errormessage(id1, '密碼太短，不得少於 '+pwlength+' 個字元！');
			return;
		}
	}
	if(strongpw) {
...
function checkinvite() {
	...
	if(invitecode.match(/<|"/ig)) {
		errormessage('invitecode', '邀請碼包含敏感字元！');
		return;
	}
	var x = new Ajax();
