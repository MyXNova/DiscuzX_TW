		...
		if (el != null) {
			...
			x.get('home.php?mod=spacecp&ac=index&op=getblock&blockname='+blockname+'&inajax=1',function(s) {
				if (s) {
					...
					el.id = blockname;
					s = s.replace(/\<script.*\<\/script\>/ig,'<font color="red"> [javascript 腳本儲存後顯示] </font>');
					el.innerHTML = s;
...
spaceDiy.extend({
	...
	showEditNvInfo : function () {
		...
		if(!nv) {
			...
			dom.innerHTML = '<span id="editnvinfo" class="edit" style="background-color:#336699;" onclick="spaceDiy.opNvEditInfo();">設定</span>';
