...
function removeindexheats() {
	return confirm('您確定要把此主題從熱門主題中移除嗎？');
}
...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = '您的文章長度不符合要求！\n\n目前長度： ' + mb_strlen(theform.message.value) + ' ' + '字元\n系統限制： ' + postminchars + ' 到 ' + postmaxchars + ' 字元';
	}
	if(s) {
		showError(s);
		doane();
...
function loadData(quiet, formobj) {
	...
	if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
		if(!quiet) {
			showDialog('沒有可以恢復的資料！', 'notice');
		}
		return;
	}

	if(!quiet && !confirm('此操作將覆蓋目前文章內容，確定要恢復資料嗎？')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">有新回覆的主題，點此檢視', 'colspan': colspan }}};
