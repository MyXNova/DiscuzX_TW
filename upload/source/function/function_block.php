						...
						if($blockitem['makethumb'] == 1) {
							...
							if($_G['setting']['ftp']['on'] && !empty($_G['setting']['ftp']['host'])) {
								...
								if($ftp->connectid && $ftp->ftp_size($thumbpath) > 0) {
									$picflag = 1; //common_block_pic 資料表中的 picflag 標識（0 本地端，1 遠端）
									...
									if(file_exists($_G['setting']['attachdir'].'./'.$thumbpath)) {
										if(ftpperm(fileext($thumbpath), filesize($_G['setting']['attachdir'].'./'.$thumbpath)) && $ftp->upload($_G['setting']['attachdir'].'/'.$thumbpath, $thumbpath)) {
											...
											$picflag = 0; //common_block_pic 資料表中的 picflag 標識（0 本地端，1 遠端）
											$_G['block_makethumb'] = true;
										}
									}
								}
...
function block_updateitem($bid, $items=array()) {
	...
	for($i = 1; $i <= $block['shownum']; $i++) {
		...
		if(!$jump) {
			...
			$curitem['makethumb'] = 0;
			if($block['picwidth'] && $block['picheight'] && $curitem['picflag']) { //picflag=0 為 url 網址
