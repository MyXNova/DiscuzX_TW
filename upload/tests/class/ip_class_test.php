...
class TestIp
{
    ...
    function test_checkaccess() {
        ...
        assertTrue(ip::checkaccess("100.64.96.69", $iplist));//IPv4，單個 IP
        assertTrue(ip::checkaccess("100.90.96.69", $iplist));//IPv4，按首碼限制
        assertFalse(ip::checkaccess("127.0.0.1", $iplist));//IPv4，未在限制清單
        assertTrue(ip::checkaccess("2408:84e1:126:5848:987b:bbca:8fb:fd78", $iplist));//IPv6，單個 IP
        assertTrue(ip::checkaccess("fe80::a1d8:428b:87a0:3a6a", $iplist));//IPv6，按首碼限制
        assertFalse(ip::checkaccess("::1", $iplist));//IPv6，未在限制清單
    }
